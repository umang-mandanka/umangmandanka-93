...
const ScrollTriggeredElement = ({
  ...
  const getIcon = () => {
    const icons = [<Code size={24} className="text-cyan-400" />, <Layers size={24} className="text-purple-400" />, <Move3d size={24} className="text-amber-400" />, <FileCode size={24} className="text-emerald-400" />, <Monitor size={24} className="text-rose-400" />, <Palette size={24} className="text-indigo-400" />, <Brush size={24} className="text-orange-400" />, <Component size={24} className="text-teal-400" />, <TerminalSquare size={24} className="text-green-400" />, <GitBranch size={24} className="text-blue-400" />, <Figma size={24} className="text-pink-400" />, <Cpu size={24} className="text-yellow-400" />, <Globe size={24} className="text-violet-400" />];
    return icons[Math.floor(offset % icons.length)];
  };

  // Choose a color based on offset
  const getBgGradient = () => {
    const gradients = ["from-purple-700/40 to-blue-600/40", "from-cyan-700/40 to-emerald-600/40", "from-amber-700/40 to-orange-600/40", "from-rose-700/40 to-pink-600/40", "from-indigo-700/40 to-violet-600/40"];
  ...
  const codeSnippets = ["const [state, setState] = useState(null);", "useEffect(() => { /* ... */ }, []);", "export default function Component() { }", "<div className='flex items-center'></div>", "@keyframes fadeIn { from {opacity:0} to {opacity:1} }"];
  ...
  return <div ref={elementRef} className={`fixed ${position === "left" ? "left-8" : "right-8"} select-none
    ...
    cursor: 'pointer'
      ...
      {isHovered && <>
          <div className="absolute inset-0 -z-10 bg-gradient-to-br from-white/5 to-transparent rounded-xl animate-pulse-glow mx-[100px]"></div>
